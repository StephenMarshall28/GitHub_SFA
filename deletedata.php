...
if(isset($_POST['delete']))
{
    ...
    if(empty($id))
    {
        echo "<script>alert('Please select a record first!');</script>";
    }
    else
    {
        $check = mysqli_query($conn, "SELECT * FROM smartfarmdatabase WHERE id='$id'");

        if(mysqli_num_rows($check) == 0)
        {
            echo "<script>alert('Invalid ID selected!');</script>";
        }
        else
        {
            $sql = "DELETE FROM smartfarmdatabase WHERE id='$id'";
